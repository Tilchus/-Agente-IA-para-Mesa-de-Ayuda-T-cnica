import json, os, config_inicial, random
...
def guardar_ticket_json(historial, area, prioridad, resumen):
    ...
    datos = []
    if os.path.exists(config_inicial.ARCHIVO_TICKETS):
        with open(config_inicial.ARCHIVO_TICKETS, "r", encoding="utf-8") as f:
            try: datos = json.load(f)
            except: datos = []
            
    datos.append(nuevo_ticket)
    
    with open(config_inicial.ARCHIVO_TICKETS, "w", encoding="utf-8") as f:
